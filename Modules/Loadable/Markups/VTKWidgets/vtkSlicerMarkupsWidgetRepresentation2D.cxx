...
vtkSlicerMarkupsWidgetRepresentation2D::ControlPointsPipeline2D::ControlPointsPipeline2D()
{
  ...
  this->LabelsLineProperty->SetLineWidth(1.);
...
void vtkSlicerMarkupsWidgetRepresentation2D::UpdateAllPointsAndLabelsFromMRML(double labelsOffset)
{
  ...
  for (int controlPointType = 0; controlPointType < NumberOfControlPointTypes; ++controlPointType)
  {
    ...
    for (int pointIndex = startIndex; pointIndex <= stopIndex; pointIndex++)
    {
      ...
      labelPos[0] = slicePos[0] + labelsOffset;
      labelPos[1] = slicePos[1] + labelsOffset;
      ...
      if (this->MarkupsDisplayNode->GetLabelConnectorLineVisibility())
      {
        ...
        double lineX = (this->MarkupsDisplayNode->GetLabelConnectorLineScale() * screenWidth) / 100.;
        double lineY = (this->MarkupsDisplayNode->GetLabelConnectorLineScale() * screenHeight) / 100.;

        // Add a function which predicts the exact extensions of the label text on the view in height and width.
        ...
        if (slicePos[0] < screenWidth /2.)
        {
          lineEndPos[0] = slicePos[0] + labelsOffset - lineX;
        ...
        if (slicePos[1] < screenHeight / 2.)
        {
          lineEndPos[1] = (slicePos[1] + labelsOffset) - lineY;
        ...
        if (lineEndPos[1] < labelTextHeight)
        {
            lineEndPos[1] = 0. + (labelTextHeight / 2.);
            labelPos[1] = 0. + (labelTextHeight / 2.);
        }
        if (lineEndPos[1] > screenHeight - labelTextHeight)
        {
            lineEndPos[1] = screenHeight - labelTextHeight;
